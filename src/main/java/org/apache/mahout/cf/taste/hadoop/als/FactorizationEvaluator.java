...
public class FactorizationEvaluator extends AbstractJob {
  ...
  public static class PredictRatingsMapper extends Mapper<LongWritable,Text,DoubleWritable,NullWritable> {
    ...
    protected void setup(Context ctx) throws IOException, InterruptedException {
      ...
      Path pathToU = new Path(conf.get(USER_FEATURES_PATH));  // 从userFeatures 里面读取 用户行为
      Path pathToM = new Path(conf.get(ITEM_FEATURES_PATH));  // 从itemFeatures 里面读取 物品信息

      U = ALS.readMatrixByRows(pathToU, conf);   // 每行的vector做为matrix的行 Map（行号, vector)
    ...
    protected void map(LongWritable key, Text value, Context ctx) throws IOException, InterruptedException {
      ...
      if (U.containsKey(userID) && M.containsKey(itemID)) {
        double estimate = U.get(userID).dot(M.get(itemID)); //(Uvector dot Mvector)
